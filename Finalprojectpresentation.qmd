...
```{r}
#| layout-ncol: 2
cnetplot(ego_up_old)+ labs(
  title = "genes connected to differentially enriched pathways in older females"
)
cnetplot(ego_down_old)+ labs(
  title = "genes connected to differentially enriched pathways in older males"
)
...
```{r}
#| layout-ncol: 2
cnetplot(ego_up_young) + labs(
  title = "genes connected to differentially enriched pathways in younger males"
)
cnetplot(ego_down_young)+ labs(
  title = "genes connected to differentially enriched pathways in younger females"
)
```

```{r}
enrichres_down_old <- pairwise_termsim(ego_down_old)
enrichres_up_old <- pairwise_termsim(ego_up_old)
enrichres_down_young <- pairwise_termsim(ego_down_young)
enrichres_up_young <- pairwise_termsim(ego_up_young)
```

```{r}
#| layout-ncol: 2
emapplot(enrichres_up_old)+ labs(title = "Network map of differentially enriched pathways in older males")
emapplot(enrichres_down_old) + labs(title = "Network map of differentially enriched pathways in older females")
```
...
emapplot(enrichres_down_young)
emapplot(enrichres_up_young)
```
